...
if __name__ == "__main__": #  https://github.com/dask/distributed/issues/2520
    ...
    bucket_dir = "/ltenas8/data/GPM_Buckets_Granules/DPR_RainySurface/"
    ...
    filepaths = get_local_filepaths(product=product, 
                                    ...
                                    end_time=end_time)
    print(f"--> {len(filepaths)} granules to process")
    ...
    def create_dataframe_from_granule(filepath):
        ...
        #---------------------------------------------------------------------.
        # Add WarmRainfall and Virga indicators
        ds["flagWarmRainfall"] = np.logical_and(da_liquid_precip_rate.sum(dim="range") > 0,
                                                da_solid_precip_rate.sum(dim="range") < 1).astype(int)
 
        ds["flagVirga"] =  np.logical_and(ds["precipRateNearSurface"] == 0,  
                                          ds["flagPrecip"] > 0,
                                          ).astype(int)
